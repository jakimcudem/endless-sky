...
const Command Command::SLEFT(ONE << 5, "Strafe left");
const Command Command::SRIGHT(ONE << 6, "Strafe right");
const Command Command::MOUSE_TURNING_HOLD(ONE << 7, "Mouse turning (hold)");
const Command Command::AIM_TURRET_HOLD(ONE << 8, "Turret aim override (hold)");
const Command Command::PRIMARY(ONE << 9, "Fire primary weapon");
const Command Command::TURRET_TRACKING(ONE << 10, "Toggle turret tracking");
const Command Command::SECONDARY(ONE << 11, "Fire secondary weapon");
const Command Command::SELECT(ONE << 12, "Select secondary weapon");
const Command Command::LAND(ONE << 13, "Land on planet / station");
const Command Command::BOARD(ONE << 14, "Board selected ship");
const Command Command::HAIL(ONE << 15, "Talk to selected ship");
const Command Command::SCAN(ONE << 16, "Scan selected ship");
const Command Command::JUMP(ONE << 17, "Initiate hyperspace jump");
const Command Command::FLEET_JUMP(ONE << 18, "Initiate fleet jump");
const Command Command::TARGET(ONE << 19, "Select next ship");
const Command Command::NEAREST(ONE << 20, "Select nearest hostile ship");
const Command Command::NEAREST_ASTEROID(ONE << 21, "Select nearest asteroid");
const Command Command::DEPLOY(ONE << 22, "Deploy / recall fighters");
const Command Command::AFTERBURNER(ONE << 23, "Fire afterburner");
const Command Command::CLOAK(ONE << 24, "Toggle cloaking device");
const Command Command::MAP(ONE << 25, "View star map");
const Command Command::INFO(ONE << 26, "View player info");
const Command Command::MESSAGE_LOG(ONE << 27, "View message log");
const Command Command::FULLSCREEN(ONE << 28, "Toggle fullscreen");
const Command Command::FASTFORWARD(ONE << 29, "Toggle fast-forward");
const Command Command::HELP(ONE << 30, "Show help");
const Command Command::PAUSE(ONE << 31, "Pause");
const Command Command::PERFORMANCE_DISPLAY(ONE << 32, "Toggle performance info");
const Command Command::FIGHT(ONE << 33, "Fleet: Fight my target");
const Command Command::HOLD_FIRE(ONE << 34, "Fleet: Toggle hold fire");
const Command Command::GATHER(ONE << 35, "Fleet: Gather around me");
const Command Command::HOLD_POSITION(ONE << 36, "Fleet: Hold position");
const Command Command::HARVEST(ONE << 37, "Fleet: Harvest flotsam");
